...
from data_mastor.utils import patch_path
...
def mock_abort(mocker: MockerFixture) -> MagicMock:
    mock = mocker.patch(patch_path(abort, middlewares))
...
class TestPrivacyCheckerDlMw:
    ...
    def mock_interface_is_up(
        ...
        mock = mocker.patch(patch_path(_interface_is_up))
    ...
    def mock_interface_ip(
        ...
        mock = mocker.patch(patch_path(_interface_ip))
    ...
    def mock_is_leaking(
        ...
        mock = mocker.patch(patch_path(_is_leaking))
...
class TestResponseSaverSPMW:
    ...
    def test_process_spider_output(self, tmpdir: Path, mock_abort: MagicMock) -> None:
        # setup
        ...
        assert res[0] == item
        assert res[1] == req2
        # follow-up request with local url
        mw.in_dir = tmpdir
        ...
        # assert res_local[1].url == mw._html_local_url(res_local[1].url)
